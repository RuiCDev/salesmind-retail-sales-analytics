...
SELECT 
    CAST(product_id AS SIGNED) AS product_id, -- Mudança para SIGNED (INT normal)
...
SELECT
    transaction_id,
    STR_TO_DATE(date, '%Y-%m-%d') AS date,
    CAST(product_id AS SIGNED) AS product_id, -- Uniformização para SIGNED
    CAST(store_id AS SIGNED) AS store_id,     -- Uniformização para SIGNED
    CAST(customer_segment AS CHAR(50)) AS customer_segment,
...
-- =========================
-- Agora todos os tipos são compatíveis (INT com INT, VARCHAR com VARCHAR)
